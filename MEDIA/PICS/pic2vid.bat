...
rem 这是一个将图片文件夹中的不同格式的图片转换为不同格式的视频的bat文件
...
rem 遍历图片文件夹中的jpg和jpeg图片，包括子文件夹
for /r "%folder%" %%a in (*.jpg *.jpeg) do (
rem 获取图片的文件名（不含后缀），去掉空格
for /f "delims=" %%b in ("%%~na") do set name=%%b
set name=!name: =!
rem 调用ffmpeg将图片转换为mp4视频，覆盖同名文件
ffmpeg -y -loop 1 -i "%folder%\!name!.jpg" -c:v libx264 -t %duration% -pix_fmt yuv420p -r %fps% "%pic2vid%\!name!.mp4"
)
rem 遍历图片文件夹中的png和webp图片，包括子文件夹
for %%i in ("%folder%\*.png" "%folder%\*.webp") do (
    rem 使用ffmpeg将图片转换为带有透明通道的mov视频，覆盖同名文件
    ffmpeg -loop 1 -i "%%i" -c:v png -t %duration% -r %fps% -pix_fmt rgba "%pic2vid%\%%~ni.mov"
)
rem 转换完成，打印提示信息
echo.
echo 转换完成！
